...
function populateOnThisPage() {
	...
	for (var i = 0; i < items.length; ++i) {
		...
		var button = $("<button type='button' class='btn btn-outline-primary'></button>").text(text);
		var a = $("<a href= #" + id + "></a>");
		a.append(button);
...
function populateListWithNavbar() {
	...
	var ul = document
		.querySelectorAll(
			"body > div.allContainer > header > div.topbar2 > div.banner > nav > ul > li.current.hasChildren > ul > li.current.hasChildren > ul"
		)[0]
